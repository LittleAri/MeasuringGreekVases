...
def binarize(
    ...
    try:
        ...
        mp_x, mp_y, npoint, spoint, wpoint, epoint = ellipseMeasurements(
            img, 0.125, ellipseLeeway, 0.05, border
        )
...
def ellipseMeasurements(img, minCol, leeway, midpointDiff, border):
    ...
    try:
        ...
        north = int(toty / border)
    try:
        south = max(np.where(img[mp1:, :] < mn)[0]) + mp1
    except:
        south = toty - (int(toty / border))
